...
parser.add_argument('--model', type=str, default='llama3_law', help='Model name for result file path.')
...
def judgment_metric():
    ...
    # 读取label
    with open('/home/hz/yangyi/LCRcheck/data/cases/Judgment/one_jud_label2.json', "r",encoding="utf8") as g:
            labels=json.load(g)
...
def confused_mtric():
    ...
    for config in file_configs:
        ...
        # 读取对应的label文件
        with open(f'/home/hz/yangyi/LCRcheck/data/cases2/confused/trainDataset/{config["label"]}', "r",encoding="utf8") as g:
                labels=json.load(g)

        # 
        with open(f'/home/hz/yangyi/LCRcheck/results/test/{args.model}/{config["file"]}', 'r', encoding='utf-8') as f:
            results=f.readlines()
            fact_abstract=[]
            for re in tqdm(results[:]):
                one=eval(re)
...
def multi_metrics():
    # 定义需要处理的文件列表
    file_list = ['multi_defendant.json','multi_defendant_final.json']
