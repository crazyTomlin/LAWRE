    ...
    def __init__(self, split,train):
        if train:
            if args.dataset=='ELAM':
                ...
                dataset = load_dataset('json',data_files='LCRCheckCode/cases2/fairness/cail_test.json') 
            elif args.dataset=='LeCaRD':
                dataset = load_dataset('json',data_files='LCRCheckCode/cases2/base/lecard_common_train.json')
...
tokenizer = AutoTokenizer.from_pretrained(
    pretrained_model_name_or_path="LCRCheckCode/DELTA_CH",
    cache_dir=None,
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.pretrained = AutoModel.from_pretrained('LCRCheckCode/DELTA_CH')
...
for epoch in range(num_epochs):
    ...
    if args.dataset=='ELAM':
        with open('LCRCheckCode/results/save/Delta/delta_elam_loss.txt','a',encoding="utf8") as f:
                    f.write(f"Epoch {epoch + 1}/{num_epochs}, Average Loss: {avg_loss:.4f}")
                    f.write("\n") 
    elif args.dataset=='CAIL':
        with open('LCRCheckCode/results/save/Delta/delta_cail_loss.txt','a',encoding="utf8") as f:
                    f.write(f"Epoch {epoch + 1}/{num_epochs}, Average Loss: {avg_loss:.4f}")
                    f.write("\n") 
    elif args.dataset=='LeCaRD':
        with open('LCRCheckCode/results/save/Delta/delta_lecard_loss.txt','a',encoding="utf8") as f:
                    f.write(f"Epoch {epoch + 1}/{num_epochs}, Average Loss: {avg_loss:.4f}")
                    f.write("\n") 
...
# 保存训练完的模型
if args.dataset=='ELAM':
    torch.save(model.state_dict(), "LCRCheckCode/results/save/Delta/delta_elam.pth")
elif args.dataset=='CAIL':
    torch.save(model.state_dict(), "LCRCheckCode/results/save/Delta/delta_cail.pth")
elif args.dataset=='LeCaRD':
    torch.save(model.state_dict(), "LCRCheckCode/results/save/Delta/delta_lecard.pth")
